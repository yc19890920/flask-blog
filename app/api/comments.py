...
from app import db
from app.blog.models import Article, BlogComment
from . import api
...
def get_comments():
    ...
    pagination = BlogComment.query.order_by(BlogComment.id.desc()).paginate(
...
def get_post_comments(id):
    ...
    page = request.args.get('page', 1, type=int)
    pagination = post.comments.order_by(BlogComment.id.asc()).paginate(
        page, per_page=current_app.config['FLASKY_COMMENTS_PER_PAGE'],
        error_out=False)
    comments = pagination.items
    prev = None
...
def new_post_comment(id):
    post = Article.query.get_or_404(id)
    comment = BlogComment.from_json(request.json)
